...
namespace Stathis
{
	...
	public class File_Manager : MonoBehaviour
	{
		...
		public static float xml_getVersion (string myXmlFile, string myXmlNode)
		{
...
*/
        ...
        public static bool IfFileExist(string filename, out string filePath)
        {
            string tempPath = GetPathOfflineData();
            tempPath = Path.Combine(tempPath, filename);
            bool isExists = File.Exists(tempPath);
            if (!isExists) { isExists = Directory.Exists(tempPath); }
            if (!isExists) { filePath = string.Empty; }
            filePath = tempPath;
            return isExists;
        }
        ...
        public static void SaveOfflineData(byte[] dataToSave, string dataFileName, Ext fileExtension)
        {
            string tempPath = GetPathOfflineData();
        ...
        public static string GetPathOfflineData()
        {
            string tempPath = Path.Combine(Application.persistentDataPath, "offlineData");
            //Create Directory if it does not exist
